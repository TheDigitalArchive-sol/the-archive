use anchor_client::solana_sdk::{signature::Keypair, signer::{EncodableKey, Signer}};
use anchor_client::{Client,Cluster};
use anchor_lang::prelude::Pubkey;
use dotenv::dotenv;
use storage_manager::{check_storage_account, create_storage_account, retrieve_stored_data, store_data_in_chunks};
use std::{env, error::Error, rc::Rc, str::FromStr, sync::Arc};
...
async fn main() -> Result<(), Box<dyn Error>> {
   ...
   let sb_program_id = Pubkey::from_str(&env::var("BOOK_STORAGE_PROGRAM_ID").expect("BOOK_STORAGE_PROGRAM_ID not set")).unwrap();
   let storage_account = Keypair::new();
   let storage_acc_kp = create_storage_account(&client,  &genesis_keypair, &storage_account, sb_program_id, &data).await;
   let _a = check_storage_account(&client, sb_program_id, storage_acc_kp).await.unwrap();
   let _k = store_data_in_chunks(&client, &genesis_keypair, storage_acc_kp, sb_program_id, &data).await;
   let stored_data = retrieve_stored_data(&client, sb_program_id, storage_acc_kp).await.unwrap();
   let decrypted_data = tokio::task::spawn_blocking(move || { light_msg_decryption(&key, stored_data).expect("Failed to decrypt data")}).await?;
